<?php  
	$traslado = $this->db->get_where('traslado',array('id_traslado'=>$ID))->row();
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-toolbar">
                        <a href="<?php echo  base_url().'admin/traslados/encabezado' ;?>"
                                    ...
                                    <form class="form" action="javascript:void(0);"
                                        ...
                                        <div class="input-group">
                                            <input type="text" name="code" class="form-control" placeholder=""
                                                aria-describedby="basic-addon2" value="<?php echo $traslado->code_traslado;?>">
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <span><?php echo date("d/m/Y", strtotime($traslado->date));?> | <?php echo date("h:i a", strtotime($traslado->date));?></span>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label><b>Responsable:</b></label><br>
                                <?php echo $this->crud_model->getName('admin', $traslado->responsable) ;?>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label><b>Origen:</b></label><br>
                                <?php echo ($traslado->Desde == 0)? "Bodega":$this->db->get_where('branch',array('branch_id'=>$traslado->Desde))->row()->name;?>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label><b>Destino:</b></label><br>
                                <?php echo ($traslado->Hacia == 0)? "Bodega":$this->db->get_where('branch',array('branch_id'=>$traslado->Hacia))->row()->name;?>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label><b>Detalles:</b></label><br>
                                <?php echo $traslado->motivo;?>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <span
                                    class="badge badge-<?php if($traslado->status==1){echo 'success';}else{echo 'danger';}?>"><?php if($traslado->status==1){echo 'Activo';}else{echo 'Eliminado';}?></span>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-padded">
                            ...
                            <tbody>
                                <?php 
                                    $prods = $this->crud_model->get_products_traslado($ID)->result_array();
									foreach($prods as $product):
                                        $prod_id = $product['products_id']; $cost = $product['cost'];
                                        if ($product['products_id_2'] > 0) $prod_id = $product['products_id_2'];
									    $pr = $this->db->get_where('products',array('products_id'=>$prod_id))->row();
                                        $amount = $product['total'];
                                        if ($product['products_id_2'] > 0 && $pr->cnt_prod_matriz > 0) $amount = $product['total'] / $pr->cnt_prod_matriz;
                                        if ($product['products_id_2'] > 0) $cost = $product['cost'] * $pr->cnt_prod_matriz;
                                ...
                                <tr>
                                    ...
                                    <td class="text-center bolder"> <span
                                            class="text-success"><?php echo $moneda.number_format($cost,2,'.',',');?></span></td>
                                    <td class="text-right bolder"><span
                                            class="text-success"><?php echo $moneda.number_format($subtotal,2,'.',',') ;?></span></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                        ...
                        <div
                            ...
                            <div class="text-right d-flex flex-column">
                                <span class="font-weight-boldest font-size-h3 line-height-sm"
                                    id="total"><?php echo $moneda.number_format($total,2,'.',',');?></span>
                            </div>
                        </div>
                    </div>
                </div>
