            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">Actividad de todos los usuarios
                    ...
                    <form class="form-inline" method="post"
                        action="<?php echo base_url().'admin/reportes/actividad/';?>">
                        <div class="card-toolbar">
                            <?php if($user_type == 1): ?>
                            <div class="dropdown dropdown-inline mr-2">
                                <select class="form-control btn btn-light-info" id='usuarios'
                                    onchange="this.form.submit()" name="user_id">
                                    <option value=''>Todos</option>
                                    <?php $users = $this->db->get('admin')->result_array(); foreach ($users as $user):?>
                                    <option value='<?php echo $user['admin_id'];?>'
                                        <?php if($user['admin_id'] == $user_id) echo 'selected'; ?>>
                                        <?php echo $user['name'].' '.$user['last_name'];?></option>
                                    <?php endforeach;?>
                                </select>
                            </div>
                            <?php endif;?>
                            <div class="my-2 my-md-0">
                                <div class="input-icon">
                                    <input type="text" class="form-control" placeholder="Buscar..." readonly id="kt_datepicker"
                                        onchange="this.form.submit()" name="date" <?php if($date != ''){?>
                                        value="<?php echo date("m/d/Y", strtotime($date));?>" <?php } ?> />
                            ...
                            <div class="dropdown-menu dropdown-menu-sm dropdown-menu-left">
                                <ul class="navi flex-column navi-hover py-2">
                                    <li
                                        ...
                                        Exportar como: <?php if($user_id == ''){$_id = 0;}else{ $_id = $user_id;} ?>
                                    </li>
                                    <li class="navi-item">
                                        <a href="<?php echo base_url().'admin/export_excel/actividades/'. $_id .'/'.$date?>"
                                            class="navi-link">
                                    ...
                                    <li class="navi-item">
                                        <a href="<?php echo base_url().'admin/export_pdf/actividades/'.$_id.'/'.$date?>"
                                            class="navi-link">
                                            <span class="navi-icon">
...
function actividad() {
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url();?>admin/search/actividad',
        data: {
            user: $user,
        },
        success: function(response) {
            jQuery('#actividad').html(response);
        },
        error: function(e) {
            console.log("ERROR : ", e);
        }
