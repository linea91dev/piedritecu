<?php $category_id = base64_decode($param2);
    ...
    $data = $this->db->get_where('mark' , array('mark_id' => $category_id))->result_array();
    foreach ($data as $row):
        $products = $this->db->get_where('products', array('mark'=>$category_id))->num_rows();
?>   
...
<script type="text/javascript" language="javascript" >  
    $(document).ready(function(){  
        var dataTable = $('#user_data').DataTable({  
            ...
            "order":[],  
            "ajax":{  
                url:"<?php echo base_url() . 'admin/get_inventario_by_marca/'.$category_id; ?>",  
                type:"POST"  
            },  
            "columnDefs":[  
                {  
