            ...
            <div style="width:100%; font-size: 16px; line-height: 24px; font-family: 'nunito'; color: #555;">
                            ...
                            <table style="width: 100%;line-height: inherit;text-align: left;">
                                <tr>
                                    ...
                                    <td style="padding-bottom: 20px; vertical-align: top;text-align:center;padding-top:5px;">
                                        <p></p>
                                        <p style="text-transform:uppercase"><b><u>Reporte de Egresos</u></b></p>
                                    </td>
                                </tr>
                            </table>
                        </td>
                ...
                <table cellpadding="0" cellspacing="0"
                    ...
                    <?php $branch_id = $this->session->userdata('branch_id');
                        $moneda = $this->crud_model->get_info("moneda"); $n=1;
                        $datoss = $this->db->order_by('expense_id','DESC')->get_where('expense', array('status'=>$status));
                    ...
                    <tr>
                        ...
                        <td style="padding:15px;font-size: 12px;border-left: 1px solid black;border-top: 1px solid black;border-right: 1px solid black;border-bottom: 1px solid black;text-align:left"
                            colspan="4">
                            <?php if($rows['origin'] == 0){ 
                                $destino = '(Caja Chica) - Efectivo';
                            }
                            else {
                                $account = $this->db->get_where('account_bank', array('account_bank_id' => $rows['origin']))->row()->name_account;
                                $bank_id = $this->db->get_where('account_bank', array('account_bank_id' => $rows['origin']))->row()->bank_id;
                        ...
                        <td colspan="4"
                            style="padding:15px; border-right: 1px solid black;border-bottom: 1px solid black; border-left: 1px solid black; border-top: 1px solid black; padding-top:15px;font-size: 15px; text-align:center;">
                            <?php echo $moneda.number_format($rows['saldo_inicial'],2,'.',',') ;?>
                        </td>
                        <td colspan="4"
                            style="padding:15px; border-right: 1px solid black;border-bottom: 1px solid black; border-left: 1px solid black; border-top: 1px solid black; padding-top:15px;font-size: 15px; text-align:center;">
                            <?php echo $moneda.number_format($rows['amount'],2,'.',',') ;?>
                        </td>
                        <td colspan="4"
                            style="padding:15px; border-right: 1px solid black;border-bottom: 1px solid black; border-left: 1px solid black; border-top: 1px solid black; padding-top:15px;font-size: 15px; text-align:center;">
                            <?php echo $moneda.number_format($rows['nuevo_saldo'],2,'.',',') ;?>
                        </td>
                    </tr>
                    <?php endforeach;?>
                </table>
