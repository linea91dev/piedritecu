


<!DOCTYPE html>
<html lang="es">

<head>
    <title>Gracias
        <?php echo strtoupper($this->db->get_where('settings', array('type'=>'name'))->row()->description);?> </title>
    <meta charset="utf-8">
    <meta name="description" content="Updates and statistics" />
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no" />
    <?php include 'topcss.php';?>
</head>
<?php $moneda = $this->crud_model->get_info("moneda"); ?>
<body id="kt_body" class="header-fixed header-mobile-fixed sidebar-enabled page-loading">
    <div class="d-flex flex-column flex-root">
        <div class="d-flex flex-row flex-column-fluid page">
            <?php include 'aside.php';?>
            <div class="d-flex flex-column flex-row-fluid wrapper" id="kt_wrapper" style='margin-right: 3rem !important;'>
                <br><br>
                <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
                    <div class="d-flex flex-column-fluid">
                        <div class="container-fluid text-center">
                            <?php $data = $this->db->get_where('shopping', array('code' => $code)) ;?>
                            <h3>¡Hola!
                                <b><?php echo strtoupper($this->db->get_where('settings', array('type'=>'name'))->row()->description);?></b>
                                te da las <b> Gracias!</b> 
                            </h3>
                        <div class="card-body" style="/* padding: 2rem 2.25rem; */padding-top: 50px;padding-bottom: 50px;">
                            <center>
                        <img src="<?php echo base_url();?>uploads/Gracias.png" style="max-width:25%">
                            </center>
                            </div>
                            
                        </div>
                    </div>
                </div>
                <?php include 'footer.php';?>
            </div>
        </div>
    </div>
    
    <div id="kt_scrolltop" class="scrolltop">
        <span class="svg-icon">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px"
                height="24px" viewBox="0 0 24 24" version="1.1">
                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <polygon points="0 0 24 0 24 24 0 24" />
                    <rect fill="#000000" opacity="0.3" x="11" y="10" width="2" height="10" rx="1" />
                    <path
                        d="M6.70710678,12.7071068 C6.31658249,13.0976311 5.68341751,13.0976311 5.29289322,12.7071068 C4.90236893,12.3165825 4.90236893,11.6834175 5.29289322,11.2928932 L11.2928932,5.29289322 C11.6714722,4.91431428 12.2810586,4.90106866 12.6757246,5.26284586 L18.6757246,10.7628459 C19.0828436,11.1360383 19.1103465,11.7686056 18.7371541,12.1757246 C18.3639617,12.5828436 17.7313944,12.6103465 17.3242754,12.2371541 L12.0300757,7.38413782 L6.70710678,12.7071068 Z"
                        fill="#000000" fill-rule="nonzero" />
                </g>
            </svg>
        </span>
    </div>
    <?php include 'scripts.php';?>

    <script type="text/javascript">
    
   // función encargada de la redirección
function redireccion() {
    window.location = "https://msbox.gt/";
}

// se llamará a la función que redirecciona después de 10 minutos (600.000 segundos)
var temp = setTimeout(redireccion, 6000);

// cuando se pulse en cualquier parte del documento
document.addEventListener("click", function() {
    // borrar el temporizador que redireccionaba
    clearTimeout(temp);
    // y volver a iniciarlo
    temp = setTimeout(redireccion, 6000);
})
      </script>

</body>

</html>